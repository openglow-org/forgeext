...
## forgeext repository rules

`forgeext` is the extension host: it verifies an extension archive (`.ffx`),
unpacks it where nothing in it can reach outside, remembers what is
installed and what the operator granted, and (as the daemon grows) runs each
package's service inside the image's sandbox and brokers its API calls.
It is a separate process from forgectrl on purpose: manifest parsing, archive
handling, and a request broker for untrusted clients do not share an address
space with the cooling engine and the supervisor.
...
- **Everything in a package comes from outside.** The manifest, the archive's
  metadata, every path and mode in the payload: each is checked against a
  closed form and refused in words when it does not fit. Nothing is
  defaulted, repaired, or skipped.
- **The capability list is closed and compile-time** (`src/caps.c`). No
  manifest can spell a privileged role or a provider kind, and a capability
  that is known but not served is refused, not ignored.
- **The bytes that are parsed are the bytes that were verified.** The
  signature is checked here over the `meta.conf` this reader took from the
  archive, and the payload is hashed against that `meta.conf`. fwup's own
  reading (`-m`, `-l`, `-V`) must agree.
- **The verifier is a product gate first.** `"ForgeFIRM extension"` and
  nothing else, no archive that lists a task, and no archive whose only
  valid signature is a firmware key's.
- **A refusal leaves nothing behind.** Staging lives under the root's `tmp/`
  and is removed on every path out. `tests/install_test.py` checks it after
  each refusal.
